...
var (
	k8sRoleBindingsSAC = helpers.ForResource(resources.K8sRoleBinding)
)
...
func (d *datastoreImpl) SearchRoleBindings(ctx context.Context, q *auxpb.Query) ([]*v1.SearchResult, error) {
	return d.searcher.SearchRoleBindings(ctx, q)
}

func (d *datastoreImpl) SearchRawRoleBindings(ctx context.Context, request *auxpb.Query) ([]*storage.K8SRoleBinding, error) {
