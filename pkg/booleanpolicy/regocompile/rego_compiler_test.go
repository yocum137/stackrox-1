	...
	for _, testCase := range testCases {
		...
		t.Run(c.desc, func(t *testing.T) {
			t.Run("on fully hydrated object", func(t *testing.T) {
				val, err := pathutil.NewAugmentedObj(c.obj).Value().GetFullValue()
				require.NoError(t, err)
				m, _ := json.MarshalIndent(val, " ", " ")
	...
	runTestCases(t, []testCase{
		{
			...
			expectedResult: addContextMatchToResult(resultWithSingleMatch("A", "A0"), "BasePtr", "<nil>"),
		},
		{
		...
		},
		{
			...
			expectedResult: addContextMatchToResult(resultWithSingleMatch("A", "asdf"), "BasePtr", "<nil>"),
		},
		{
		...
		},
		{
			...
			expectedResult: addContextMatchToResult(resultWithSingleMatch("A", "A0"), "BasePtr", "anything"),
		},
