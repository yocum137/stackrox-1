...
type UnsafeSearcher interface {
	Search(q *auxpb.Query, opts ...SearchOption) ([]search.Result, error)
	Count(q *auxpb.Query, opts ...SearchOption) (int, error)
}
...
type UnsafeSearcherImpl struct {
	SearchFunc func(q *auxpb.Query, opts ...SearchOption) ([]search.Result, error)
	CountFunc  func(q *auxpb.Query, opts ...SearchOption) (int, error)
}

// Search implements Search of `UnsafeSearcher` interface.
func (f UnsafeSearcherImpl) Search(q *auxpb.Query, opts ...SearchOption) ([]search.Result, error) {
	return f.SearchFunc(q, opts...)
}

// Count implements Count of `UnsafeSearcher` interface.
func (f UnsafeSearcherImpl) Count(q *auxpb.Query, opts ...SearchOption) (int, error) {
...
func WrapUnsafeSearcherAsSearcher(searcher UnsafeSearcher) search.Searcher {
	return search.FuncSearcher{
		SearchFunc: func(_ context.Context, q *auxpb.Query) ([]search.Result, error) {
			return searcher.Search(q)
		},
		CountFunc: func(_ context.Context, q *auxpb.Query) (int, error) {
